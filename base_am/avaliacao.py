    ...
    def carrega_resultados_existentes(self):
        try:
            resultados = pd.read_csv(
                f"resultados/{self.nom_experimento}.csv", index_col="id"
            )
        except ValueError:
        ...
        for fold_num in num_folds:
            study = optuna.create_study(
                study_name=f"{self.nom_experimento}_fold_{fold_num}",
                sampler=self.sampler,
                direction="maximize",
                storage=f"sqlite:///resultados/optuna_studies.db",
                load_if_exists=True,
            ...
            self._resultados.append(
                Resultado(resultados_fold["true_y"], predicted_list)
            )
    ...
    def __call__(self, trial: optuna.Trial) -> float:
        ...
        for fold_validacao in self.fold.arr_folds_validacao:
            resultado = metodo.eval(
                self.preproc_method,
                fold_validacao.df_treino,
                fold_validacao.df_data_to_predict,
                self.fold.col_classe,
            )
