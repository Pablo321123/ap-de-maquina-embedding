        ...
        with codecs.open("embeddings_data/" + str_dataset, encoding="utf-8") as f_e:
            try:
                for i, line in enumerate(f_e):
                    ...
                    # obtem o embedding
                    dict_embedding[word] = np.array(
                        arr_line[-embedding_size:], dtype=np.float16
                    )
...
def plot_words_embeddings_vectors(
    word_vectors, color, vectors_names=[], create_figure=True
):
...
def plot_words_embeddings(dict_embedding, words_to_use):
    ...
    plot_words_embeddings_vectors(
        word_vectors, "blue", words_to_use, create_figure=True
    )
...
class Analogy:
    def __init__(self, dict_embedding, kdtree_file, overwrite_kdtree=False):
        ...
        self.kdtree_embedding = KDTreeEmbedding(
            dict_embedding, kdtree_file, overwrite_kdtree
        )

    def calcula_embedding_analogia(
        self, palavra_x: str, esta_para: str, assim_como: str
    ) -> np.array:
...
class KDTreeEmbedding:
        ...
        if overwrite_kdtree or file_not_found:
            with open(f"assignment_cache/{kdtree_file}.p", "wb") as kd_file:
                ...
                for word, embedding in dict_embedding.items():
                    ...
                    i += 1
                self.kd_embedding = KDTree(
                    self.mat_embedding, leaf_size=30, metric="euclidean"
                )
                pickle.dump(
                    {
                        "kd": self.kd_embedding,
                        "pos_to_word": self.pos_to_word,
                        "word_to_pos": self.word_to_pos,
                    },
                    kd_file,
                )

    def positions_to_word(
        self, nearest_dist: List, positions: List, words_to_ignore: List = []
    ):
    ...
    def get_most_similar_embedding(
        self,
        query: Union[np.array, str],
        k_most_similar: int = 5,
    ...
    def get_embeddings_by_similarity(
        self, query: np.array, max_distance: float, words_to_ignore: List = []
    ):
        embedding = query
        if type(query) == str:
            if query not in self.dict_embedding:
