...
class InstanceWisePreprocess(PreprocessDataset):
    ...
    def generate_preproc_test(
        self, df_text_reviews: pd.DataFrame, class_col: str = None
    ):
        data_per_text = {}
        ...
        for word_in_text in words_in_text:
            ...
            for key_word, list_syms in self.keywords_features.items():
                ...
                for word_to_search in words_to_search:
                    ...
                    val_distance = distance.cosine(
                        embedding_word_in_text, embedding_to_search
                    )
...
class AggregateEmbeddings:
    ...
    def __call__(self, text):
        ...
        for word in words_in_text:
            if (
                word not in self.dict_embedding
                or word in self.words_to_filter
                or (
                    len(self.words_to_consider) > 0
                    and word not in self.words_to_consider
                )
            ):
                continue
...
class BagOfWords(PreprocessDataset):
    def __init__(self, nome, text_col="text", stop_words=None, words_to_consider=None):
        ...
        self.vectorizer = TfidfVectorizer(
            norm="l2", stop_words=stop_words, vocabulary=words_to_consider
        )
        self.text_col = text_col

    def generate_preproc_train(
        self, df_data: pd.DataFrame, class_col: str
    ) -> pd.DataFrame:
